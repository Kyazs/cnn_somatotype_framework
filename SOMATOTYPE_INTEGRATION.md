...
│   ├── avatar_somatotype.py         # Enhanced Avatar with somatotype
│   └── ...
├── photos2avatar.py                 # Main workflow (updated)
├── somatotype_config.py             # Configuration settings
├── test_somatotype.py              # Comprehensive testing
├── simple_somatotype_test.py       # Simple validation test
...
### 1. Basic Usage with Photos2Avatar Workflow

The main `photos2avatar.py` workflow now automatically includes somatotype predictions:

```bash
# Run the main pipeline (now includes somatotype)
...
- `somatotype_measurements_female_fromImg_summary.txt` - Human-readable summary
