...
### Project Purpose
This project implements a **deep learning-based methodology for realistic human shape reconstruction from 2D images** with advanced **somatotype measurement prediction**. The system takes two photographs (front and side views) along with basic anthropometric data (gender, height, weight) and generates:

1. **3D Human Avatars** - Accurate body mesh models in Wavefront OBJ format
2. **Somatotype Measurements** - 8 specialized anthropometric measurements for body composition analysis
3. **Literature-Based Predictions** - Scientifically validated equations from peer-reviewed research

### Core Problem Domain
- **Computer Vision & 3D Modeling**: Converting 2D image data into accurate 3D human body representations
- **Anthropometric Measurement Extraction**: Using hybrid CNN-MLP networks to predict 21+ body measurements from visual data
- **Somatotype Analysis**: Body composition assessment through specialized skinfold, bone breadth, and circumference measurements
...
│   │   ├── avatar.py               # Standard avatar generation
│   │   └── avatar_somatotype.py    # Enhanced avatar with somatotype predictions
│   ├── paper_scripts/              # Research validation scripts
│   ├── photos2avatar.py            # Standard 3D avatar pipeline
│   ├── photos2somatotype.py        # Specialized somatotype analysis pipeline
│   ├── somatotype_config.py        # Configuration for somatotype measurements
