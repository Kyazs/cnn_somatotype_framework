...
def process_silhouettes(ds_name, blender_files_dir):
    ...
    silhoDB_files_dir_rsz = os.path.join(
        SIL_FILES_DIR_bw_rsz, f"silhouettes_{ds_name}_bw"
    )
    try:
...
def createDBsilhouettes(src_dir, files_dir):
    ...
    for model in file_list:
        ...
        # Load the image
        img = skimage.io.imread(os.path.join(src_dir, model))[
            :, :, :3
        ]  # [:,:,:3] to remove alpha channel

        img = rgb2gray(img)
        img = skimage.transform.rotate(img, -90)

        # Obtain the optimal threshold value
        thresh = skimage.filters.threshold_mean(
            img
        )  # threshold_otsu(img)  threshold_mean(img)
        # Apply thresholding to the image
        binary_global = img > thresh

        # Convert to uint8 explicitly to silence the warning
        binary_global = skimage.util.img_as_ubyte(binary_global)
